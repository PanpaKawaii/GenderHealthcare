...
export default function ParameterManager() {
    ...
    const [formData, setFormData] = useState({ name: '', unit: '', referenceMin: '', referenceMax: '' });
    ...
    const EditParameter = async (e) => {
        ...
        try {
            setLoading(true);
            const ParameterData = await putData(`/parameters/${editingParam._id}`, token, EditParameterData);
    ...
    return (
        <div className='container'>
            ...
            <form className='form' onSubmit={AddParameter}>
                ...
                <input placeholder='Unit' required value={formData.unit} onChange={(e) => setFormData({ ...formData, unit: e.target.value })} />
                <input type='number' placeholder='Reference Min' value={formData.referenceMin} onChange={(e) => setFormData({ ...formData, referenceMin: e.target.value })} />
                <input type='number' placeholder='Reference Max' value={formData.referenceMax} onChange={(e) => setFormData({ ...formData, referenceMax: e.target.value })} />
                    ...
                    {parameters.length === 0 ? (
                        ...
                        parameters.map((param, index) => (
                            <tr key={param._id}>
                                ...
                                <td>{param.name}</td>
                                <td>{param.unit}</td>
                                <td>{(param.referenceMin ?? 'N/A')} - {(param.referenceMax ?? 'N/A')}</td>
                ...
                <div className='modal-overlay'>
                    <div className='modal'>
                        ...
                        <form onSubmit={EditParameter}>
                            <input value={editingParam.name} onChange={(e) => setEditingParam({ ...editingParam, name: e.target.value })} required />
                            <input value={editingParam.unit} onChange={(e) => setEditingParam({ ...editingParam, unit: e.target.value })} required />
                            <input type='number' value={editingParam.referenceMin || ''} onChange={(e) => setEditingParam({ ...editingParam, referenceMin: e.target.value })} />
                            <input type='number' value={editingParam.referenceMax || ''} onChange={(e) => setEditingParam({ ...editingParam, referenceMax: e.target.value })} />
